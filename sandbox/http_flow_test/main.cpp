...
static void runOne(const std::string& name, const std::string& raw) {
  ...
  ParseResult parsed = HttpRequestParser::parse(raw);
  ...
  HttpResponse response = HttpHandlerFactory::handleRequest(parsed.request);
  std::string wire = response.toString();

  std::cout << "[RESPONSE OBJECT] status=" << response.status_code << " "
...
static void runPipeline() {
  ...
  ParseResult first = HttpRequestParser::parse(buffer);
  ...
  ParseResult second = HttpRequestParser::parse(buffer);
