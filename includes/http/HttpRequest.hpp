...
class HttpRequest {
 public:
  ...
  static ParsedRequest parse(const std::string& buffer);
 ...
 private:
  ...
  static bool _hasToken(const std::string& csvValue, const std::string& token);
  static ParseStatus _parseChunkedBody(const std::string& buffer,
                                       size_t body_start, std::string& outBody,
                                       size_t& outConsumed);
};
