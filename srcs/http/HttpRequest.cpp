...
bool HttpRequest::_hasToken(const std::string& csvValue, const std::string& token) {
...
ParseStatus HttpRequest::_parseChunkedBody(const std::string& buffer,
  ...
  while (true) {
    ...
    if (line_end == std::string::npos) return PARSE_INCOMPLETE;
    ...
    if (size_line.empty()) return PARSE_ERROR;
    ...
    if (chunk_size == 0) {
      while (true) {
        ...
        if (trailer_end == std::string::npos) return PARSE_INCOMPLETE;
        if (trailer_end == pos) {
          outConsumed = trailer_end + 2;
          return PARSE_OK;
        }
        std::string trailer = buffer.substr(pos, trailer_end - pos);
        if (trailer.find(':') == std::string::npos) return PARSE_ERROR;
        pos = trailer_end + 2;
      }
    }

    if (buffer.length() < pos + chunk_size + 2) return PARSE_INCOMPLETE;
...
ParsedRequest HttpRequest::parse(const std::string& buffer) {
  ...
  if (method_str.empty() || uri_full.empty() || version.empty() || !extra.empty()) {
  ...
  if (teIt != result.request.headers.end()) {
    ...
    size_t consumed = 0;
    ParseStatus s = _parseChunkedBody(buffer, body_start, unchunked, consumed);
    if (s != PARSE_OK) {
      result.status = s;
      return result;
    }
