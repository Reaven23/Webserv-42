...
ServerSocket::ServerSocket(int port) {
    ...
    //                   as opposed to SOCK_DGRAM for UDP which is unreliable
    //                   and unordered)
    //   - 0           : let the OS pick the default protocol (TCP for
    //   SOCK_STREAM)
    _fd = socket(AF_INET, SOCK_STREAM, 0);
    if (_fd == -1) {
        throw runtime_error(string("socket(): Fatal error: ") +
                            strerror(errno));
    }
    ...
    // INADDR_ANY (= 0.0.0.0) means "accept connections on all network
    // interfaces" (localhost, wifi, ethernet, etc.).
    addr.sin_addr.s_addr = INADDR_ANY;

    // bind() attaches the socket to the address (IP + port) defined above.
    //   - serverServerSocket                    : the socket fd to configure
    //   - (struct sockaddr*)&serverAddress : cast required because bind()
    //   expects
    ...
    if (!setNonBlocking(_fd) || !setCloseOnExec(_fd)) {
